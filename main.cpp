...
int main() {
    ...
    int numCitizens;
    std::cout << "Enter the number of citizens you would like to have: ";
    std::cin >> numCitizens;

    for (int i = 0; i < numCitizens; ++i) {
        citizens.push_back(Worker(&government, true, 50000.0 + i * 10000.0)); // Add more workers
    }
    ...
    switch (buildingChoice) {
        ...
        case 2: {
            CommericalBuildingFactory commercialFactory;
    ...
    int serviceChoice;
    std::cout << "Select a service to request:\n";
    ...
    if (airports) {
        ...
        std::cout << "Enter flight name to book: ";
        std::cin.ignore();
        std::getline(std::cin, flightName);
        airports->addFlight(flightName);
    }

    // Apply for train rescheduling
    Trains* trains = dynamic_cast<Trains*>(cityManager.getService("Trains"));
    if (trains) {
        std::string trainName;
        std::cout << "Enter train name to reschedule: ";
        std::cin.ignore();
        std::getline(std::cin, trainName);
        trains->scheduleTrain(trainName);
    }

    // Morning: Start the day with economic growth strategy
    printSectionHeader("Morning: Economic Growth Strategy");
    government.setStrategy(&economicGrowthStrategy);
    government.executeStrategy();
    printCitizenSatisfaction(citizens);

    // Morning: Manage services
    printSectionHeader("Morning: Manage Services");
    cityManager.manageServices();

    // Morning: Allocate budgets
    printSectionHeader("Morning: Allocate Budgets");
    cityManager.allocateBudget("Health", 50000.0);
    cityManager.allocateBudget("Education", 30000.0);
    cityManager.allocateBudget("Entertainment", 20000.0);
    cityManager.allocateBudget("Security", 40000.0);
    ...
    // Morning: Request services
    printSectionHeader("Morning: Request Services");
    cityManager.requestService("Health");
    cityManager.requestService("Education");
    cityManager.requestService("Entertainment");
    cityManager.requestService("Security");
    ...
    // Morning: Test HealthServices
    printSectionHeader("Morning: Test HealthServices");
    HealthServices* healthServices = dynamic_cast<HealthServices*>(cityManager.getService("Health"));
    if (healthServices) {
        healthServices->admitPatient("John Doe");
        healthServices->admitPatient("Jane Doe");
    ...
    // Morning: Test EducationServices
    printSectionHeader("Morning: Test EducationServices");
    EducationServices* educationServices = dynamic_cast<EducationServices*>(cityManager.getService("Education"));
    if (educationServices) {
        educationServices->scheduleClass("Math 101");
        educationServices->scheduleClass("Science 101");
    ...
    // Morning: Test EntertainmentServices
    printSectionHeader("Morning: Test EntertainmentServices");
    EntertainmentServices* entertainmentServices = dynamic_cast<EntertainmentServices*>(cityManager.getService("Entertainment"));
    if (entertainmentServices) {
        entertainmentServices->organizeEvent("Music Festival");
        entertainmentServices->organizeEvent("Movie Night");
    ...
    // Morning: Test SecurityServices
    printSectionHeader("Morning: Test SecurityServices");
    SecurityServices* securityServices = dynamic_cast<SecurityServices*>(cityManager.getService("Security"));
    if (securityServices) {
        securityServices->respondToEmergency("City Park");
        securityServices->conductPatrol();
    ...
    // Morning: Test TransportationServices and its subsystems
    printSectionHeader("Morning: Test TransportationServices");
    TransportationServices* transportationServices = dynamic_cast<TransportationServices*>(cityManager.getService("Transportation"));
    if (transportationServices) {
        transportationServices->manageSubsystems();
    }

    // Morning: Test PublicTransit
    printSectionHeader("Morning: Test PublicTransit");
    PublicTransit* publicTransit = dynamic_cast<PublicTransit*>(cityManager.getService("PublicTransit"));
    if (publicTransit) {
        publicTransit->addVehicle("InterCity");
        publicTransit->announceDelay(10);
    ...
    // Morning: Test Trains
    printSectionHeader("Morning: Test Trains");
    Trains* trains1 = dynamic_cast<Trains*>(cityManager.getService("Trains"));
    if (trains1) {
        trains1->addTrain("Express Train");
        trains1->scheduleTrain("City Route");
    }

    // Morning: Test Airports
    printSectionHeader("Morning: Test Airports");
    Airports* airports1 = dynamic_cast<Airports*>(cityManager.getService("Airports"));
    if (airports1) {
        airports1->addFlight("Flight 101");
        airports1->scheduleFlightMaintenance();
    }

    // Morning: Test Roads
    printSectionHeader("Morning: Test Roads");
    Roads* roads = dynamic_cast<Roads*>(cityManager.getService("Roads"));
    if (roads) {
        roads->repairRoad();
        roads->expandLanes(2);
    ...
    // Afternoon: Apply Social Welfare Strategy
    printSectionHeader("Afternoon: Social Welfare Strategy");
    government.setStrategy(&socialWelfareStrategy);
    government.executeStrategy();
    printCitizenSatisfaction(citizens);

    // Afternoon: Citizens acknowledge receiving grants
    printSectionHeader("Afternoon: Citizens Acknowledge Grants");
    for (auto& citizen : citizens) {
        citizen.acknowledgeGrant();
    }

    // Afternoon: Collect taxes
    printSectionHeader("Afternoon: Collect Taxes");
    government.payTaxes();
    printCitizenSatisfaction(citizens);

    // Afternoon: Collect levies
    printSectionHeader("Afternoon: Collect Levies");
    cityManager.payLevies(citizens);
    printCitizenSatisfaction(citizens);

    // Afternoon: Execute commands
    printSectionHeader("Afternoon: Execute Commands");
    government.setCommand(&payTaxes);
    government.executeCommand();
    printCitizenSatisfaction(citizens);
    ...
    // Evening: Apply High Population Strategy
    printSectionHeader("Evening: High Population Strategy");
    government.setStrategy(&highPopulationStrategy);
    government.executeStrategy();
    printCitizenSatisfaction(citizens);

    // Evening: Increase taxes and levies
    printSectionHeader("Evening: Increase Taxes and Levies");
    std::cout << "Satisfaction before increasing Taxes : "; printCitizenSatisfaction(citizens);  std::cout << std::endl;
    for (int i = 0; i < 6; ++i) {
        government.increaseTaxesAndLevies();
        for (auto& citizen : citizens) {
            citizen.updateSatisfaction();
    ...
    // Evening: Continuously lower taxes and levies
    printSectionHeader("Evening: Lower Taxes and Levies");
    std::cout << "Satisfaction before Lowering Taxes : "; printCitizenSatisfaction(citizens);  std::cout << std::endl;
    for (int i = 0; i < 3; ++i) {
        government.reduceLevies();
        government.setTaxRate(government.getTaxRate() * 0.9);
        for (auto& citizen : citizens) {
    ...
    // Evening: Apply Economic Growth Strategy to stabilize levies and taxes
    printSectionHeader("Evening: Stabilize Levies and Taxes");
    government.setStrategy(&economicGrowthStrategy);
    government.executeStrategy();
            for (auto& citizen : citizens) {
    ...
    // Night: Test WastePlant, SewagePlant, WaterPlant, and PowerPlant
    printSectionHeader("Night: Test Utilities");
    ...
    // Toggle plant states
    int plantChoice;
    std::cout << "Select a plant to manage:\n";
    ...
    switch (plantChoice) {
        case 1:
            switch (stateChoice) {
                ...
                case 3:
                    ...
                    break;
                default:
                    std::cout << "Invalid state choice. Exiting...\n";
                    return 1;
            }
            wasteManager.reportStatus();
            break;
        ...
        case 4:
            ...
            break;
        default:
            std::cout << "Invalid plant choice. Exiting...\n";
            return 1;
    }
